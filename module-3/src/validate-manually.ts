...
async function fetchRepos(): Promise<GithubRepo[]> {
    const response = await fetchJSON<GithubRepo[]>(apiURL)
    if (!(response instanceof Array)) {
        throw Error("Expected array")
    }
    for (let repo of response) {
        if (typeof repo.name !== "string") throw Error("Name missing or invalid")
        if (typeof repo.stargazers_count !== "number") throw Error("stargazers_count missing or invalid")
    }
    return response
}
